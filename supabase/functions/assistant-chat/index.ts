...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { message, conversationId, assistantType, userContext }: ChatRequest = await req.json();
    if (!message) {
      throw new Error("Message is required");
    }
    if (!assistantType) {
      throw new Error("Assistant type is required");
    }
    ...
    if (userContext && assistantType === "excursion_creator") {
      const contextInfo = [
    ...
    const assistantId = assistantType === "health_coach"
      ? Deno.env.get("HEALTH_COACH_ASSISTANT_ID")
      : Deno.env.get("EXCURSION_ENGINE_ASSISTANT_ID");

    if (!assistantId) {
      throw new Error(`${assistantType === "health_coach" ? "Health Coach" : "Excursion Creator"} Assistant ID not configured`);
    }
    ...
    if (assistantType === "excursion_creator") {
